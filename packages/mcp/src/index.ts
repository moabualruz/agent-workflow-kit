...
import { z } from "zod";
import { createWorkflowCommandService } from "../../core/src/index";

export type WorkflowMcpToolName =
  | "workflow"
  | "workflow_run"
  | "workflow_status"
  | "workflow_resume"
...
export type WorkflowMcpToolResult = {
  content: Array<{ type: "text"; text: string }>;
};

export type WorkflowMcpTool = {
  name: WorkflowMcpToolName;
  description: string;
...
export const workflowMcpTools: WorkflowMcpTool[] = [
  {
    name: "workflow",
...
export async function callWorkflowMcpTool(name: string, input: Record<string, unknown> = {}): Promise<WorkflowMcpToolResult> {
...
export function createWorkflowMcpServer(options: { name: string; version: string }): McpServer {
  ...
  for (const tool of workflowMcpTools) {
    server.registerTool(
      ...
      async (input) => callWorkflowMcpTool(tool.name, input),
    );
  }
...
export async function connectWorkflowMcpServer(options: { name: string; version: string }): Promise<void> {
  const server = createWorkflowMcpServer(options);
  const transport = new StdioServerTransport();
  await server.connect(transport);
}

function jsonContent(value: unknown): WorkflowMcpToolResult {
  return {
