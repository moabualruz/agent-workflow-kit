...
Effort annotations (e.g. Codex `xhigh`/`high`) are host-side reasoning settings, not part of the model id. Only the OpenCode/Pi rows are enforced in-repo (`approvedModelAliasMaps` + contract tests); the other rows were verified against vendor docs and local CLI caches on 2026-06-09 and churn quickly, so re-verify before pinning (the Grok Build CLI exposes `grok-build` and `grok-composer-2.5-fast`, while the xAI API flagship is `grok-4.3`; Antigravity also offers the `antigravity-preview-05-2026` managed-agent endpoint). Claude Code and Hermes Agent are native-reference harnesses: they resolve the tier aliases themselves, so no env mapping is needed there. Smoke-test defaults (`approvedModelAliasMaps` in `scripts/headless-smoke.ts`) use the OpenCode/Pi rows above. Pi may use OpenCode or Grok models as fallback candidates when needed. OpenCode defaults stay limited to OpenCode Zen/OpenCode Go, subscription-allowed, and free models. `deepseek-v4-pro` was removed from the Zen catalog in June 2026 and must not be pinned; `nemotron-3-super-free` was superseded by `nemotron-3-ultra-free`.
